...
const UserDashboard = () => {
  ...
  const fetchChartTheme = async () => {
    try {
      const res = await api.get('/charts/theme');
      setChartTheme(res.data.theme || 'default');
    } catch { }
  };
  ...
  const fetchIntegrationStatus = async () => {
    try {
      ...
      setIntegrationStatus(res.data.integrations || {});
    } catch { }
  };
  ...
  const handleBookmarkChat = async (message) => {
    try {
      const title = `AI Chat — ${new Date().toLocaleDateString('en-IN', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}`;
      await api.post('/charts/notes', { title, content: message.content });
      toast({ title: 'Bookmarked', description: 'Chat message saved to Notes' });
    } catch {
      toast({ title: 'Error', description: 'Failed to bookmark message', variant: 'destructive' });
    }
  };
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {[
                  ...
                  <div key={plan.name} className={`bg-gray-800 rounded-xl p-5 border ${plan.name === 'Business Pro' ? 'border-purple-500' : 'border-gray-700'}`}>
                    ...
                    <p className="text-2xl font-bold text-white mb-3">₹{plan.price}<span className="text-sm text-gray-400">/year</span></p>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between mb-2">
              <div />
              <ChartThemeSelector currentTheme={chartTheme} onThemeChange={setChartTheme} />
            </div>
            ...
            {uploadedFiles.length > 0 ? (
              uploadedFiles.map(file => (
                <div key={file.id} data-testid={`file-charts-${file.id}`}>
                  ...
                  {chartsData[file.id] ? (
                    <AnalyticsDashboard charts={chartsData[file.id].charts} filename={file.filename} themeKey={chartTheme} />
                  ) : (
                  ...
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 ${msg.role === 'user' ? 'bg-purple-600 text-white rounded-tr-sm' : msg.isError ? 'bg-red-900/20 border border-red-800 text-gray-300 rounded-tl-sm' : 'bg-gray-800 text-gray-200 border border-gray-700 rounded-tl-sm'}`}>
                    ...
                    {msg.role === 'assistant' && !msg.isError && (
                      <button
                        ...
                        className="mt-1.5 flex items-center gap-1 text-[10px] text-gray-500 hover:text-purple-400 transition-colors opacity-0 group-hover/msg:opacity-100"
                        data-testid={`bookmark-chat-${i}`}
                        title="Save to Notes"
                      >
