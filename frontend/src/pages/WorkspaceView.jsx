...
const WorkspaceView = ({ workspace, onBack, user }) => {
  ...
  const handleSendMessage = async () => {
    ...
    setChatInput('');
    setChatMessages(prev => [...prev, { role: 'user', content: DOMPurify.sanitize(userMsg), timestamp: new Date().toISOString() }]);
    setIsSending(true);
    ...
    try {
      ...
      setChatMessages(prev => [...prev, {
        role: 'assistant',
        content: DOMPurify.sanitize(String(error)),
        timestamp: new Date().toISOString()
      }]);
    } finally {
      setIsSending(false);
  ...
  return (
    // your JSX code here, using sanitized data
    // For example:
    // <div>{chatMessages.map(msg => <p key={msg.timestamp} dangerouslySetInnerHTML={{ __html: msg.content }} />)}</div>
  );
};
