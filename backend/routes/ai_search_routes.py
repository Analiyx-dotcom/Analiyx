...
async def ai_chat(req: ChatMessage, user_id: str = Depends(get_current_user_id)):
    ...
    system_msg = (
        "You are Analiyx AI, a data analytics assistant. Analyze the user's uploaded data.\n"
        "Rules: Be specific with numbers/percentages. Use markdown formatting. Use INR (₹). "
        "Build on conversation history. If data is missing, say what to upload.\n\n"
        f"DATA:\n{data_context}\n{ws_context}"
    )
    ...
    try:
        ...
        if "budget" in error_msg and "exceeded" in error_msg:
            raise HTTPException(status_code=402, detail="AI budget exceeded. Please add balance (Profile > Universal Key > Add Balance).")
